...
Vec_f vec_diff(Vec_f input){ //caluculating the diff of an vector elements
  Vec_f output;
...
Vec_f cum_sum(Vec_f input){ // cumulative sum.
  Vec_f output;
  float temp = 0;
...
private:
  Vec_f calc_s(Vec_f x, Vec_f y){
    ...
    for(unsigned int i=0; i<dx.size(); i++){
      ds.push_back(std::sqrt(dx[i]*dx[i] + dy[i]*dy[i])); // ds is the distance of two adjacent points
    }
